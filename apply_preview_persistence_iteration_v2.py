...
if 'React.Dispatch<' in s and 'import React' not in s:
    s = s.replace(
...
marker = '''    {editing&&<div className="adjust-overlay" onClick={()=>setEditing(null)}>
...
replacement = '''    {editing&&<div className="adjust-overlay" onClick={()=>setEditing(null)}>
      <div className="adjust-panel" onClick={e=>e.stopPropagation()}>
        <span className="rule-tag">局部调整 · {editing.name}</span>
        <h3>告诉 AI 只改哪里。</h3>
        <p>提交后窗口会立即关闭，当前预览会扫光等待新结果；其它方案仍可继续浏览。</p>
...
s = rep(s, marker, replacement, "文字编辑弹窗")

old_preview_start = s.index('function DesignPreview(')
old_preview_end = s.index('\n\n\n\nfunction MockupStage', old_preview_start)
...
s = s[:old_preview_start] + new_preview + s[old_preview_end:]
...
.dark .material-artboard{background:#c8cbce}
.material-plane{
...
print("✓ 第四步结果返回后保持")
print("✓ 浅灰画板 + 真实物料比例")
print("✓ 直接编辑 headline / subline / microcopy")
